...
def nx_to_pyg(nx_graph, node_scaling='none', edge_scaling='none', fit_scaler=True):
    ...
    if node_scaling == 'standard':
        if fit_scaler or node_scaler is None or (last_update_time is not None and (current_time - last_update_time) > update_interval):
            ...
            if fit_scaler:
                logging.info("Node Features (x) after initial standard scaling.")
            else:
                logging.info("Node Features (x) after periodic standard scaling.")
            ...
            logging.info("Node Features (x) after online standard scaling.")
        else:
        ...
        if fit_scaler or node_scaler is None or (last_update_time is not None and (current_time - last_update_time) > update_interval):
            ...
            x = torch.tensor(node_scaler.fit_transform(x), dtype=torch.float)
            if fit_scaler:
                logging.info("Node Features (x) after initial min-max scaling.")
            else:
                logging.info("Node Features (x) after periodic min-max scaling.")
            last_update_time = current_time
        elif node_scaler is not None:
            x = torch.tensor(node_scaler.transform(x), dtype=torch.float)
            logging.info("Node Features (x) after online min-max scaling.")
        else:
            logging.warning("No node scaler fitted. Skipping min-max scaling.")
    ...
    if edge_attr is not None:
        if edge_scaling == 'standard':
            if fit_scaler or edge_scaler is None or (last_update_time is not None and (current_time - last_update_time) > update_interval):
                ...
                if fit_scaler:
                    logging.info("Edge Attributes (edge_attr) after initial standard scaling.")
                else:
                    logging.info("Edge Attributes (edge_attr) after periodic standard scaling.")
            elif edge_scaler is not None:
                edge_attr = torch.tensor(edge_scaler.transform(edge_attr), dtype=torch.float)
                logging.info("Edge Attributes (edge_attr) after online standard scaling.")
            else:
            ...
            if fit_scaler or edge_scaler is None or (last_update_time is not None and (current_time - last_update_time) > update_interval):
                ...
                edge_attr = torch.tensor(edge_scaler.fit_transform(edge_attr), dtype=torch.float)
                if fit_scaler:
                    logging.info("Edge Attributes (edge_attr) after initial min-max scaling.")
                else:
                    logging.info("Edge Attributes (edge_attr) after periodic min-max scaling.")
            elif edge_scaler is not None:
                edge_attr = torch.tensor(edge_scaler.transform(edge_attr), dtype=torch.float)
                logging.info("Edge Attributes (edge_attr) after online min-max scaling.")
            else:
                logging.warning("No edge scaler fitted. Skipping min-max scaling for edges.")
...
def update_nx_graph(nx_graph, update_dot_file):
    ...
    for node_id in update_graph.nodes():
        if nx_graph.has_node(node_id):
            ...
            logging.debug(f"Updated node: {node_id} with attributes {update_graph.nodes[node_id]}.")
        else:
    ...
    for u, v, key, data in update_graph.edges(data=True, keys=True):
        ...
        logging.debug(f"Added edge: '{u}' -> '{v}' with key={key} and attributes {data}.")
